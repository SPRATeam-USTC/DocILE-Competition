...
def single_process(args):
    ...
    for image_path in tqdm.tqdm(image_paths):
            ...
            if len(ann_data_list)==0:
                ...
                for j,ann_data in enumerate(ann_data_list):
                    ...
                    if compute_iou(ann_box, ocr_box, w, h) > threshold and ann_label not in tmp_label:
                        ...
                        in_item.append(ann_data["line_item_id"])
        ...
        item_ids = list(set(x for item_id in line_item_id for x in item_id if x!= "null"))
        ...
        if len([x["line_item_id"] for x in ann_data_list]) == 0:
            item_label_list_tmp = []
        else:
            item_label_list_tmp = [[] for _ in range(max([x["line_item_id"] for x in ann_data_list])+1)]
        for i, item_id in enumerate(item_ids):
            for j, line_item_id_li in enumerate(line_item_id):
                if item_id in line_item_id_li:
                    item_label_list_tmp[item_id].append(j)
        ...
        item_label_list = []
        item_label_with_instance_list = []
        for j,item_label_tmp in enumerate(item_label_list_tmp):
            item_label = []
            item_label_with_instance = []
            for i,instance_label in enumerate(instance_label_list):
                if all(elem in item_label_tmp for elem in instance_label):
                    item_label.append(instance_label)
                    item_label_with_instance.append(i)

            
            item_label_list.append(item_label)
            item_label_with_instance_list.append(item_label_with_instance)
        sum_num += len(instance_label_list)
        ...
        info = dict(image_name=os.path.abspath(image_path), polys=polys, contents=contents, labels=labels, sentences_embed_path=output_embed_path, \
            instance_label=instance_label_list, instance_class=instance_class_list, line_item_label=item_label_list, line_item_label_with_instance=item_label_with_instance_list)
