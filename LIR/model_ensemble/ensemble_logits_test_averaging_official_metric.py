...
def cal_classify_metrics(threshold, predictions_origin, labels_origin):
    ...
    # data = {
    #     'class name': [i for j, i in enumerate(vocab.key_words[:-1]) if j not in [7,18,22]],
...
def select_best_step(step_num, metrics_step_thre_path):
    ...
    for step, metrics in metrics_step_thre.items():
        ...
        # p_lst= [x[1] for x in metric_lst]
        # r_lst= [x[2] for x in metric_lst]
...
if __name__ == "__main__":
    ...
    logits /= (step_num*2)
    prediction = copy.deepcopy(logits)
