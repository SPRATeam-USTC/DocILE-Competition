...
from .utils_docile import align_logits, extract_merge_feats, extract_merge_feats_v2, parse_merge_labels, select_items_entitys_idx, decode_merge_logits, cal_tp_total
import libs.configs.doclie_config_ocr_data_kie_merge36 as cfg
...
class AttentionMerger(nn.Module):
    ...
    def forward(self, feats, masks=None):
        keys = self.key_layer(feats) # (b, c, l) -> (b, c, l)
        querys = self.query_layer(feats) # (b, c, l) -> (b, c, l)
        logits = torch.einsum('bck,bcq->bkq', keys, querys) * self.scale # K.T * Q -> (b, l_score, l_sample)
        if masks is not None:
            logits = logits - (1 - masks[:, :, None]) * 1e8
        return logits
...
class GraphDocForTokenClassification(LayoutLMv2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab = cfg.Docliekie_vocab
        self.merge_head = AttentionMerger(config.hidden_size, config.hidden_size)
        ...
        if self.training: 
            ...
            for class_idx, class_name in enumerate(self.vocab.key_words):
                ...
                if sum([len(batch_li) for batch_li in specific_class_instance_class]) != 0:
                    have_instance = True
                    entity_features, merge_mask = extract_merge_feats_v2(sequence_output, specific_class_instance_label, classify_logits)
                    merger_logits = self.merge_head(entity_features, merge_mask)
                    merge_labels, merge_label_mask = parse_merge_labels(sequence_output, specific_class_instance_label)  #制作label（B, ocr_box.num in img_instances, ocr_box.num in img_instances）
                    raw_merge_loss = F.binary_cross_entropy_with_logits(merger_logits, merge_labels, reduction='none')
                    sum_merge_loss += (raw_merge_loss * merge_label_mask).sum()
                    sum_merge_mask += merge_label_mask.sum()
            if have_instance:
                merge_loss = sum_merge_loss / sum_merge_mask
            else:
                merge_loss = torch.tensor(0.0, requires_grad=True, device=classify_logits.device)
            ...
            loss = classify_loss + merge_loss

            if not return_dict:
                output = (output_logits.sigmoid(), ) + outputs[2:] 
                return ((loss,classify_loss,merge_loss) + output) if loss is not None else output
        
        if not self.training: 
            specific_class_metrics = torch.zeros((classify_logits.size(0), 0, 3), device=classify_logits.device)
            ...
            for class_idx, class_name in enumerate(self.vocab.key_words):
                ...
                merge_labels, merge_label_mask = parse_merge_labels(sequence_output, specific_class_instance_label)  #制作label（B, ocr_box.num in img_instances, ocr_box.num in img_instances）
                valid_items_polys_idxes = select_items_entitys_idx(class_idx, classify_logits, attention_mask, cfg.classify_threshold)  # 对ocr_box,先看他属于哪个类别，把class in ["NAME", "CNT", "PRICE", "PRICE&CNT", "CNT&NAME"]的挑出来
                ...
                if sum([len(batch_li) for batch_li in merge_mask]) != 0:
                    ...
                    pack_tp_pred_gt_num = cal_tp_total(pred_items_polys_idxes, specific_class_instance_label, device=merger_logits.device)
                    specific_class_metrics = torch.cat([specific_class_metrics, pack_tp_pred_gt_num.unsqueeze(1)], dim=1)
                else:
                    pack_tp_pred_gt_num = torch.zeros((classify_logits.size(0), 3), device=classify_logits.device)
                    pack_tp_pred_gt_num[:,-1] = torch.tensor([len(x) for x in specific_class_instance_class])
                    specific_class_metrics = torch.cat([specific_class_metrics, pack_tp_pred_gt_num.unsqueeze(1)], dim=1)
            loss = torch.tensor(0.0)

            if not return_dict:
                output = (specific_class_metrics, output_logits.sigmoid()) + outputs[2:]
                return ((loss,) + output) if loss is not None else output
