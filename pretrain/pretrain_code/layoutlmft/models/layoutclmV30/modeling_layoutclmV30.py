...
from transformers.models.layoutlm.modeling_layoutlm import LayoutLMPooler as LayoutCLMv30Pooler
...
LayoutCLMv30LayerNorm = torch.nn.LayerNorm
...
class LayoutCLMv30Embeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings."""

    def __init__(self, config):
        super(LayoutCLMv30Embeddings, self).__init__()
        ...
        self.LayerNorm = LayoutCLMv30LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class VisualTokenExtractor(nn.Module):
    def __init__(self, config):
        ...
        load_state_info = self.backbone.load_state_dict(torch.load(config.vision_pretrain)['state_dict'], False)
        logger.warn("while loading vision backbone from %s \n missing_keys: %s \n unexpected_keys: %s" % (config.vision_pretrain, \
            ','.join(load_state_info.missing_keys), ','.join(load_state_info.unexpected_keys)))
...
class LayoutLMv30SelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LayoutLMv30Attention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = LayoutLMv30SelfAttention(config)
...
class LayoutLMv30Layer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = LayoutLMv30Attention(config)
...
class LayoutLMv30Encoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.layer = nn.ModuleList([LayoutLMv30Layer(config) for _ in range(config.num_hidden_layers)])
...
class LayoutCLMv30Model(LayoutLMv2PreTrainedModel):
    def __init__(self, config):
        super(LayoutCLMv30Model, self).__init__(config)
        self.config = config
        self.has_visual_segment_embedding = config.has_visual_segment_embedding
        self.embeddings = LayoutCLMv30Embeddings(config)
        ...
        self.encoder = LayoutLMv30Encoder(config)
        self.pooler = LayoutCLMv30Pooler(config)
...
class LayoutCLMv30ForPretrain(LayoutLMv2PreTrainedModel):
    config_class = LayoutCLMv30Config
    def __init__(self, config):
        super().__init__(config)
        self.layoutclm = LayoutCLMv30Model(config)
...
class LayoutCLMv30ForTokenClassification(LayoutLMv2PreTrainedModel):
    config_class = LayoutCLMv30Config
    def __init__(self, config):
        super().__init__(config)
...
class LayoutCLMv30ForClassification(LayoutLMv2PreTrainedModel):
    config_class = LayoutCLMv30Config
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class LayoutCLMv30ForRelationExtraction(LayoutLMv2ForRelationExtraction):
    config_class = LayoutCLMv30Config
    def __init__(self, config):
        super().__init__(config)
        self.layoutclm = LayoutCLMv30Model(config)
