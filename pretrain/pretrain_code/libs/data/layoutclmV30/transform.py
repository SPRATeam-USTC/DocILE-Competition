...
class CallMlmTarget:
    ...
    '''
    def __init__(self, mask_embed, is_cover, mlm_prob=0.15, random_token_prob=0.1, leave_unmasked_prob=0.1):
    ...
    def __call__(self, image, input_sentences_embed, input_bboxes, dtc_labels, loader, data_idx):
        ...
        if self.is_cover:
            cover_bboxes = input_bboxes[mask].tolist()
...
class CallMvmTarget:
    ...
    '''
    def __init__(self, use_mvm=False, mvm_prob=0.088):
    ...
    def __call__(self, image, input_sentences_embed, input_bboxes, unmask_sentences_embed, mlm_mask, lcl_labels, dtc_labels, bdp_labels):
        if self.use_mvm:
            sz = len(input_sentences_embed)
            mask = np.random.rand(sz) < self.mvm_prob
            mask = np.logical_and(mask, ~mlm_mask) # remove mlm tokens
            mask[0] = False # remove [CLS]
            
            # if not mask.any(): # select none
            #     select_mask = ~mlm_mask.astype('bool')
            #     select_mask[0] = False
            #     select_range = np.where(select_mask==True)[0] # avail mask for masked vision model
            #     if select_range.shape[0] > 0:
            #         np.random.shuffle(select_range) # shuffle select
            #         mask[select_range[0]] = True # random select one token as mask input ids
            
            mvm_mask = mask.astype('int64')
            ...
            mvm_mask = np.zeros_like(mlm_mask).astype('int64')
            unmask_image = image
        
        return image, input_sentences_embed, input_bboxes, unmask_sentences_embed, mlm_mask, lcl_labels, dtc_labels, bdp_labels, unmask_image, mvm_mask